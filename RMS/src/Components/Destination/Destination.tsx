...
const Source: React.FC<SourceProps> = ({ applicationId }) => {
  ...
  const fetchDestinations = async (page = 1, pageSize = 10, query = "", filters = {}) => {
    try {
      setLoading(true);
      const { data } = await axios.get(`http://localhost:3000/api/destinations/${applicationId}`, {
  ...
  const handleSave = async (updatedItems: any[]) => {
    try {
      const requests = updatedItems.map(item => {
        ...
        return axios.put(`http://localhost:3000/api/destinations/${destinationid}`, {
          destinationid, alias, url, apikey,isactive, isdeleted
        });
      });
  ...
  const generateBaseColumns = (data: any[]) => {
    ...
    return Object.keys(sample)
      .filter(key => key !== 'destinationid' &&key !== 'applicationid' && key !== "isactive" && key !== "isdeleted" && key !== "status")
      .map((key) => (
        {
          accessorKey: key,
