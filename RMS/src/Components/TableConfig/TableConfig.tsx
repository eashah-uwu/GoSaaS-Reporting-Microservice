...
const TableConfig: FC<TableConfigProps> = ({
  ...
  const handleDeleteConfirm = () => {
    setTableData((prevData) =>
      prevData.map((dataItem) =>
        dataItem.applicationid === selectedDataId
  ...
  const filteredData = tableData.filter((app: any) => app.status !== "delete");
  return (
    <>
      <Box padding={6} sx={{ width: "90%", margin: "0 auto" }}>
        {filteredData && (
          <Table data={filteredData} columns={columns} pageSize={pageSize} />
        )}
        <Box
          sx={{
            display: "flex",
        ...
        >
          ...
          <IconButton
            onClick={handleAdd}
            sx={{ ml: 2, width: "auto", height: "auto" }}
          >
